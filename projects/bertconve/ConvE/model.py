...
class ConvE_channel(torch.nn.Module):
    def __init__(self, args, num_entities, num_relations):
        super(ConvE_channel, self).__init__()
        self.emb_e = torch.nn.Embedding(num_entities, args.embedding_dim, padding_idx=0)
        self.emb_rel = torch.nn.Embedding(num_relations, args.embedding_dim, padding_idx=0)
        self.inp_drop = torch.nn.Dropout(args.input_drop)
        self.hidden_drop = torch.nn.Dropout(args.hidden_drop)
        ...
        # self.bn1 = torch.nn.BatchNorm2d(32)
        # self.bn2 = torch.nn.BatchNorm1d(args.embedding_dim)
        self.conv1 = torch.nn.Conv2d(1, args.conv_channel, (args.kernel_size, args.kernel_size), 1, 0, bias=args.use_bias)
        self.bn0 = torch.nn.BatchNorm2d(1)
        self.bn1 = torch.nn.BatchNorm2d(args.conv_channel)
        self.bn2 = torch.nn.BatchNorm1d(args.embedding_dim)
        self.register_parameter('b', Parameter(torch.zeros(num_entities)))
        self.fc = torch.nn.Linear(args.hidden_size,args.embedding_dim)
        print(num_entities, num_relations, flush=True)
...
class BertConvE(torch.nn.Module):
    def __init__(self, args, num_entities, num_relations):
        super(BertConvE, self).__init__()
        # self.emb_e = torch.nn.Embedding(num_entities, args.embedding_dim, padding_idx=0)
        self.emb_rel = torch.nn.Embedding(num_relations, args.embedding_dim, padding_idx=0)
        self.inp_drop = torch.nn.Dropout(args.input_drop)
        self.hidden_drop = torch.nn.Dropout(args.hidden_drop)
        ...
        self.bn2 = torch.nn.BatchNorm1d(args.embedding_dim)
        self.register_parameter('b', Parameter(torch.zeros(num_entities)))
        hidden_size = args.conv_channel * (self.emb_dim1 * 2 - args.kernel_size + 1) * (self.emb_dim2 - args.kernel_size + 1)
        assert hidden_size == args.hidden_size, f'args.hidden_size (= {args.hidden_size}) not as expected (={hidden_size})'
        self.fc = torch.nn.Linear(args.hidden_size, args.embedding_dim)
        print(f'num_entities={num_entities}, num_relations={num_relations}', flush=True)

    def init(self):
        # xavier_normal_(self.emb_e.weight.data)
        xavier_normal_(self.emb_rel.weight.data)
        # self.emb_e = torch.nn.Embedding.from_pretrained(emb_pretrained) # default will freeze
    
    def forward(self, emb_bert, e1, rel):
        # e1_embedded= self.emb_e(e1).view(-1, 1, self.emb_dim1, self.emb_dim2)
        e1_embedded= emb_bert(e1).view(-1, 1, self.emb_dim1, self.emb_dim2)
        rel_embedded = self.emb_rel(rel).view(-1, 1, self.emb_dim1, self.emb_dim2)
        ...
        x = F.relu(x)
        # x = torch.mm(x, self.emb_e.weight.transpose(1,0))
        x = torch.mm(x, emb_bert.weight.transpose(1,0))
        x += self.b.expand_as(x)
        pred = torch.sigmoid(x)
